...
export function PoliciesView() {
  const { account, financeAdmin, owners, mandates, requests, paused, run, busy, refresh, toast, chainOk = true } = useApp();
              ...
              <section className="governance-history">
                ...
                <div className="policy-capability-panel" aria-label="Policy capabilities">
                  ...
                  <div className="policy-capability-grid">
                    ...
                    <div><b>Activate draft</b><p>Safe threshold action that makes a Draft active.</p><button type="button" className="btn" disabled={!chainOk || !capabilities.canActivate || !!busy} onClick={() => governance(`Activate mandate #${policy.id}`, 'activateMandate', [policy.id])}>Activate 2-of-2</button>{!isSafeOwner && <small>Requires current Safe owner</small>}{capabilities.activateBlockedReason && <small>{capabilities.activateBlockedReason}{activationBlockingRequest ? ` Request #${activationBlockingRequest.id} is still ${REQUEST_STATES[activationBlockingRequest.state] ?? 'in flight'}.` : ''}</small>}</div>
                    <div><b>Retire mandate</b><p>Safe threshold action; in-flight requests must be resolved first.</p><button type="button" className="btn danger" disabled={!chainOk || !capabilities.canRetire || !!busy} onClick={() => governance(`Retire mandate #${policy.id}`, 'retireMandate', [policy.id])}>Retire 2-of-2</button>{capabilities.retireBlockedReason && <small>{capabilities.retireBlockedReason}</small>}</div>
                    <div><b>{paused ? 'Resume module' : 'Emergency pause'}</b><p>{paused ? 'Only the Safe can widen access again.' : 'Finance Admin may tighten spending immediately.'}</p>{paused ? <button type="button" className="btn primary" disabled={!chainOk || !capabilities.canResume || !!busy} onClick={() => governance('Resume all mandates', 'unpauseAll', [])}>Resume 2-of-2</button> : <button type="button" className="btn danger" disabled={!chainOk || !capabilities.canPause || !!busy} onClick={pauseModule}>Pause module</button>}<small>{paused ? 'Requires current Safe owner' : 'Requires Finance Admin'}</small></div>
